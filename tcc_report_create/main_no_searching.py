...
# If default WAS selected
    ...
    cord_glob = '*Coordination Results*Recommendations*.pdf'
    ...
    if len(base_glob_list) > 1:
        eprint('Found multiple versions of TCC-Base_v#. Please remove all extras.')
        exit()
    ...
    # TODO Load a file browser and ask the user to choose where the file is
        ...
        eprint('Found no version of TCC-Base_v#. Please provide a sheet')
        exit()
    else:
    ...
    if len(rec_glob_list) > 1:
        eprint('Found multiple versions of TCC-Rec_v#. Please remove all extras.')
        exit()
    elif len(rec_glob_list) == 0:
        eprint('Found no version of TCC-Rec_v#. Assuming only Base.')
    else:
